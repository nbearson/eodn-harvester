#!/usr/bin/python3
#
# EODN_Harvest_0_1_13.py
#
# Harvest data from USGS EROR
#   and populate EODN
#
...
class Criteria():  # base class for additionalCriteeria
...
class Process():
    ...
    def upload_to_eodn(self, xnd_file, file):
        ...
        results = call(['lors_upload', '--duration=10h', '--copies=1', '--depot-list', '-o', xnd_file, file])
        if results != 0:
            exit(1)
        else:
            return 0
    ...
    def add_to_avmss(self, file_path, file_name):
        ...
        call(['scp', '/home/ssec/EODN/process_eodn.sh', file_name])
        return result
...
def fix_wsdl(wsdlurl):  # fix errors in USGS WSDL
    ...
    with urllib.request.urlopen(url=wsdlurl) as request:
...
def main():
    ...
    while run is True:
        ...
        print('number found: ' + str(len(search.get_attribs('entities').item)))
        ...
        for entity in search.get_attribs('entities').item:
            ...
            for product in products:
                ...
                if product.get_attribs('available'):
                    ...
                    if config['glovis'] is True and (product.get_attribs('download_code') == 'STANDARD'):
                        logger.write('       sending scene id to GloVis')
                    ...
                    if config['avmss'] is True and (product.get_attribs('download_code') == 'FR_BUND'):
                        logger.write('        sending LandsatLook image to AVMSS')
                        result = process.add_to_avmss(file_path=file_path, file_name=file_name)
                        if result != 0:
                            logger.write_error('Line 865: avmss copy failed for ' + base_name)
                            logger.write_error('result')
