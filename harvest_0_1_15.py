...
class Process():
    ...
    def download(self, url, file_path, usgs_filesize):
        ...
        try:
            ...
            logger.write('file size: ' + file_size)
    ...
    def upload_to_eodn(self, xnd_file, file):
        ...
        for i in range(retry):
            try:
                results = call(['lors_upload', '--duration=10h', '--copies=1', '-H=dlt.incntre.iu.edu', '--verbose=1', '-o', xnd_file, file])
                if results == 0:
                    break
            except Exception as e:
                logger.write_error(e)
        ...
        for entity in search.get_attribs('entities').item:
            ...
            for product in products:
                ...
                if product.get_attribs('available'):
                    ...
                    print('glovis: '+str(config['glovis']))
